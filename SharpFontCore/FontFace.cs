...
namespace SharpFont
{
    ...
    public sealed class FontFace
    {
        ...
        /// </summary>
        /// <param name="pixelSize">The desired size of the font, in pixels.</param>
        /// <returns>The glyph data that the font support.</returns>
        public List<Glyph> GetGlyphs(float pixelSize)
        {
            List<Glyph> allGlyphs = new List<Glyph>();
            List<CodePoint> codePoints = GetSupportedCodePoints();
            foreach (var codePoint in codePoints)
            {
                allGlyphs.Add(GetGlyph(codePoint, pixelSize));
            }
            return allGlyphs;
        }
        ...
        /// <param name="pixelSize">The desired size of the font, in pixels.</param>
        /// <returns>The glyph data that the font support.</returns>
        public Dictionary<CodePoint, Glyph> GetGlyphsDictonary(float pixelSize)
        {
            var allGlyphs = new Dictionary<CodePoint, Glyph>();
            List<CodePoint> codePoints = GetSupportedCodePoints();
            foreach (var codePoint in codePoints)
            {
                Glyph glyph = GetGlyph(codePoint, pixelSize);
                if (glyph.RenderWidth == 0 || glyph.RenderHeight == 0)
                    continue;
                allGlyphs.Add(codePoint, glyph);
            }
            return allGlyphs;
        }

        /// <summary>
        /// Gets supported CodePoints.
        /// </summary>
        /// <returns>The supported CodePoints as List<CodePoint>.</returns>
        public List<CodePoint> GetSupportedCodePoints()
        {
            return new List<CodePoint>(charMap.Table.Keys);
        }

        /// <summary>
        /// Gets supported characters.
        /// </summary>
        /// <returns>The supported characters as List<Char>.</returns>
        ...
        /// Gets supported characters.
        /// </summary>
        /// <returns>The supported characters as string.</returns>
        public string GetSupportedCharactersString()
        {
            List<CodePoint> codePoints = GetSupportedCodePoints();
