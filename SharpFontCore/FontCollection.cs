...
namespace SharpFont
{
    ...
    public sealed class FontCollection
    {
        ...
        private Dictionary<string, List<Metadata>> _fontTable = new Dictionary<string, List<Metadata>>();
        public Dictionary<string, List<Metadata>> FontTable => _fontTable;
        ...
        public FontFace Load(string family, FontWeight weight = FontWeight.Normal, FontStretch stretch = FontStretch.Normal, FontStyle style = FontStyle.Regular)
        {
            ...
            if (!_fontTable.TryGetValue(family.ToLowerInvariant(), out sublist))
                return null;
        ...
        /// <returns>The loaded font if it exists in the collection; otherwise, <c>null</c>.</returns>
        public FontFace Load(string fileName)
        {
            List<Metadata> values = new List<Metadata>();
            foreach (var element in _fontTable)
            {
                values.AddRange(element.Value);
            }
            if (values.Count == 0)
                return null;
        ...
        void AddFile(Metadata metadata, bool throwOnError)
        {
            ...
            if (_fontTable.TryGetValue(key, out sublist))
                sublist.Add(metadata);
            else
                _fontTable.Add(key, new List<Metadata> { metadata });
        }
